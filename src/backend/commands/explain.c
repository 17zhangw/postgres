			...
			ExplainState *es)
{
		...
		planstate->instrument && planstate->instrument->nloops > 0)
	{
		...
		else
		{
			...
			ExplainPropertyFloat("Operator Time", "us", planstate->instrument->differenced_total, 3, es);
	...
	else if (es->analyze)
	{
		...
		else
		{
			...
			ExplainPropertyFloat("Operator Time", "us", 0.0, 3, es);
			ExplainPropertyBool("Operator Stopped", planstate->bytejack_stop, es);
			ExplainPropertyInteger("Plan Node Id", NULL, planstate->plan->plan_node_id, es);
		}
	}
	...
	if (es->workers_state && es->verbose)
	{
		...
		for (int n = 0; n < w->num_workers; n++)
		{
			...
			else
			{
				...
				ExplainPropertyFloat("Actual Rows", NULL, rows, 0, es);
				ExplainPropertyFloat("Actual Loops", NULL, nloops, 0, es);
				ExplainPropertyFloat("Operator Time", "us", instrument->differenced_total, 3, es);
				ExplainPropertyBool("Operator Stopped", planstate->bytejack_stop, es);
				ExplainPropertyInteger("Plan Node Id", NULL, planstate->plan->plan_node_id, es);
			}
