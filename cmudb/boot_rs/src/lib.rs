...
#[no_mangle]
pub extern "C" fn redis_connect(redis_host: *const c_char) -> *mut RedisCon {
    let rhost = unsafe { CStr::from_ptr(redis_host) }.to_str().unwrap();
    let client = redis::Client::open(rhost).unwrap();
