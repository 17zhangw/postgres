...
PG_FUNCTION_INFO_V1(warmer_prewarm);
...
static int64 warm(FunctionCallInfo fcinfo, PrewarmType ptype, Relation rel, ForkNumber forkNumber, char *forkString)
{
	int64		first_block;
	int64		last_block;
	int64		nblocks;
 ...
 */
Datum
warmer_prewarm(PG_FUNCTION_ARGS)
{
	Oid			relOid;
	text	   *forkName;
	text	   *type;
    ...
    part = RelationGetPartitionDesc(rel, false);
    if (part != NULL && part->nparts > 0)
    {
        int i;
        for (i = 0; i < part->nparts; i++)
        {
            Relation child = relation_open(part->oids[i], AccessShareLock);
            blocks_done += warm(fcinfo, ptype, child, forkNumber, forkString);
            relation_close(child, AccessShareLock);
        }
    }
    else
    {
        blocks_done += warm(fcinfo, ptype, rel, forkNumber, forkString);
    }
