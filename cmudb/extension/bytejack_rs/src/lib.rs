...
use pgrx_pg_sys::StatementTimeout;
use sig::compute_plan_signature;
use sig::compute_settings_signature;
...
impl RedisCacheEntry {
    ...
    pub fn should_rerun(&mut self) -> bool {
        ...
        if self.fetches <= self.backoff_counter {
            ...
            self.fetches = 0;
            self.backoff_counter = std::cmp::max(self.backoff_counter * 2, 100);
            true
        }
    }
...
#[no_mangle]
pub extern "C" fn redis_connect(conn: *const c_char) -> *mut RedisCon {
    let client = redis::Client::open(unsafe { CStr::from_ptr(conn) }.to_str().unwrap()).unwrap();
...
impl OperatorState {
    ...
    fn try_stop(&mut self) -> bool {
        ...
        (mean_old - 2.0 * stdev_old <= mean_new) && (mean_new <= mean_old + 2.0 * stdev_old)
    }
    ...
    fn add_tuple_batch(&mut self, n_tuples: f64, accumulated_us: f64) -> i32 {
        ...
        match self.plan_type {
            _ => {
                ...
                let start_new_batch = (self.current_n_tuples >= 0.10 * self.optimizer_plan_rows) && (accumulated_us >= 1e6);
                if start_new_batch {
...
#[no_mangle]
pub extern "C" fn cache_get_explain(redis_con: *mut RedisCon, pi: *mut List, plan: *mut Plan, query_text: *const c_char) -> *mut CacheResult {
    //let now = std::time::Instant::now();

    let settings = unsafe { compute_settings_signature() };
    let mut qplan = String::new();
    unsafe { compute_plan_signature(pi, plan, &mut qplan); }
    qplan.push_str(format!("; Settings: {};", settings).as_str());
    qplan.push_str(unsafe { CStr::from_ptr(query_text) }.to_str().unwrap());
    ...
    //if intelligent {
    //    let should_rerun = match redis_con.stats.get_mut(qplan) {
    ...
    let val: Option<String> = redis::cmd("GET").arg(qplan.clone()).query(con).unwrap();
    match val {
        None => { return ptr::null_mut(); }
        Some(s) => {
            ...
            if let serde_json::Value::Array(ref mut veccy) = v {
                if let serde_json::Value::Object(ref mut map) = veccy[0] {
                    ...
                    map.insert("Cache Key".into(), serde_json::Value::String(qplan.clone()));
                }
            }
            else if let serde_json::Value::Object(ref mut map) = v {
                map.insert("Bytejack".into(), serde_json::Value::String("true".into()));
                map.insert("Cache Key".into(), serde_json::Value::String(qplan.clone()));
            }
...
#[no_mangle]
pub extern "C" fn cache_append_explain(redis_con: *mut RedisCon, pi: *mut List, plan: *mut Plan, query_text: *const c_char, explain_text: *const c_char, ok: bool) {
    let settings = unsafe { compute_settings_signature() };
    let mut qplan = String::new();
    unsafe { compute_plan_signature(pi, plan, &mut qplan); }
    qplan.push_str(format!("; Settings: {};", settings).as_str());
    ...
    if !ok {
        ...
        if let serde_json::Value::Object(ref mut map) = v {
            ...
            unsafe { map.insert("Execution Time".into(), serde_json::Value::Number(StatementTimeout.into())) };
        }
        let s = serde_json::to_string(&v).unwrap();
        redis::cmd("SET").arg(qplan).arg(s).execute(con);
    } else {
        let explain = unsafe { CStr::from_ptr(explain_text) }.to_str().unwrap();
        redis::cmd("SET").arg(qplan).arg(explain).execute(con);
    }
