...
fn main() -> eyre::Result<()> {
    ...
    if let Some(_) = env_tracked("CARGO_FEATURE_CPG15") {
        let mut bindings_file = build_paths.src_dir.clone();
        bindings_file.push(&format!("cpg15.rs"));

        match fs::metadata(bindings_file) {
            Ok(_) => return Ok(()),
            Err(_) => {},
        }
    }
    ...
    let pg_configs: Vec<(u16, PgConfig)> = if is_for_release {
        ...
        for pgver in SUPPORTED_VERSIONS() {
            ...
            else if let Some(_) = env_tracked(&format!("CARGO_FEATURE_CPG{}", pgver.major)) {
                found.push(pgver);
            }
        }
...
fn generate_bindings(
    ...
    let dest_dirs = if is_for_release || env_tracked(&format!("CARGO_FEATURE_CPG{}", major_version)).is_some() {
    ...
    for dest_dir in dest_dirs {
        ...
        if let Some(_) = env_tracked(&format!("CARGO_FEATURE_CPG{}", major_version)) {
            bindings_file.push(&format!("cpg{}.rs", major_version));
        } else {
        ...
        write_rs_file(
            ...
            quote! {
                use crate as pg_sys;
                #[cfg(any(feature = "pg12", feature = "pg13", feature = "pg14", feature = "pg15", feature = "cpg15", feature = "pg16"))]
        ...
        if let Some(_) = env_tracked(&format!("CARGO_FEATURE_CPG{}", major_version)) {
            oids_file.push(&format!("cpg{}_oids.rs", major_version));
        } else {
...
fn add_blocklists(bind: bindgen::Builder) -> bindgen::Builder {
    bind.blocklist_type("(Nullable)?Datum") // manually wrapping datum types for correctness
        ...
        .blocklist_function("plpgsql.*")
        .blocklist_function("LocalXLogReaderRoutine")
        .blocklist_function("zopen")
        .blocklist_function("BIG5toCNS")
        .blocklist_function("CNStoBIG5")
        .blocklist_function("pfctlinput")
        .blocklist_function("alloca")
        .blocklist_function("__[A-Z].*") // these are reserved and unused by Postgres
        .blocklist_function("__[a-z].*") // these are reserved and unused by Postgres
}
