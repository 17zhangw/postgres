...
char* bytejack_redis = "localhost:6379";
...
Datum bytejack_connect(PG_FUNCTION_ARGS) {
  redis_con = redis_connect(bytejack_redis);
...
void _PG_init(void) {
  ...
  DefineCustomStringVariable("bytejack.redis", "Redis Connection", NULL, &bytejack_redis, "localhost:6379", PGC_USERSET, 0, NULL, NULL, NULL); 
  ...
  //bytejack_prev_ExplainOneQuery_hook = ExplainOneQuery_hook;
  //ExplainOneQuery_hook = bytejack_ExplainOneQuery;
