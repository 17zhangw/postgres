...
char *generate_normalized_query(JumbleState *jstate, const char *query, int query_loc, int *query_len_p) {
  ...
  if (start != NULL) {
	  ...
	  memcpy(tmp_query, norm_query, start - norm_query);
	  memcpy(tmp_query + (start - norm_query), end + 2, n_quer_loc - ((end + 2) - norm_query));
	  *query_len_p = (start - &query[query_loc]) + (query_len - ((end + 2) - &query[query_loc]));
	  return tmp_query;
  }
  return norm_query;
}
